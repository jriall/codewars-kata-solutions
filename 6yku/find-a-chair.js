// https://www.codewars.com/kata/the-office-v-find-a-chair/javascript
...
function meeting(x, need){
  ...
  let count = need;
  let chairs = x.reduce((a,b)=>{
    let temp = b[1] - b[0].length
    if (count<=0) {
      return a;
    }
    if (temp > 0) {
      if (temp > count) {
        ...
        a.push(b[1] - b[0].length);
      }
      count-=b[1] - b[0].length
    } else {
      a.push(0);
    }
    return a;
  },[]);
  return count > 0 ? "Not enough!" : chairs;
}